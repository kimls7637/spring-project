								...
								<div>
									...
									<div class="mt-1">
										<input id="password1" type="password" required
											autocomplete="current-password"
											class="block w-full appearance-none rounded-md border border-gray-500/30 px-3 py-2 text-sm placeholder-gray-500/80 shadow-sm focus:border-gray-500 focus:outline-none focus:ring-0 dark:bg-gray-500/20 sm:text-base"
											placeholder="특수문자, 한글 제외한 8~16자" name="pw1" maxlength="16"
											style="width: 80%;">
									</div>
								</div>
								<div>
									...
									<div class="mt-1">
										<input id="email" type="text" autocomplete="email"
											...
											<div
											style="width: auto; display: inline-block; padding-top: 7px; padding-bottom: 7px; border-radius: 7px; margin-left: 9px;">
											<button type="button" class="phonebutton" id="eMailCheck">중복
												체크</button>
										</div>
									</div>
								</div>
								...
								<div>
									...
									<div class="mt-1">
										<input id="nickName" type="text" required
											class="block w-full appearance-none rounded-md border border-gray-500/30 px-3 py-2 text-sm placeholder-gray-500/80 shadow-sm focus:border-gray-500 focus:outline-none focus:ring-0 dark:bg-gray-500/20 sm:text-base"
											placeholder="알파벳, 한글, 숫자 3글자 이내" name="mNickname"
											style="width: 80%; display: inline-block;" maxlength="10">
										<div
											style="width: auto; display: inline-block; padding-top: 7px; padding-bottom: 7px; border-radius: 7px; margin-left: 9px;">
	...
	<script data-nscript="afterInteractive">
		...
		$('#nickCheck').click(function() {
			...
			const gongbak = /\s/g;
			  ...
			  if (checkNick.test(nickCheck))
				    return alert('닉네임은 알파벳, 한글, 숫자만 3자리 이내로 가능합니다.')
			  if (gongbak.test(nickCheck))
					return alert('공백이 포함되어 있습니다.')
			if(nickCheck.length ==0){
				return	alert('닉네임을 입력해주세요.');
			}
			  if(nickCheck.length > 3){
				  return alert('3글자 이내로 입력해주세요.');
			  }
			$.ajax({
				url : 'nickCheck.do',
				type : 'post',
